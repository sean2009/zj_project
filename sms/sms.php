...
class Sms{
	...
	*				 'expectSendTime'=>$expectSendTime);
	*/
	public function sendWapPush($paras){
		try {
			
			//第一个参数是方法名，第二个参数是数组(所有的方法的参数都放到数组中)
			if ($paras['expectSendTime']) {
				$result = $this->client->call('SendWapPush2', $paras);
			}
			else {
				$result = $this->client->call('SendWapPush1', $paras);
			}
	...
	public function addTask($paras){
		try {
			...
			if ($paras['beginTime']&&$paras['endTime']) {
				$result = $this->client->call('AddTask2', $paras);
			}
			else {
				$result = $this->client->call('AddTask1', $paras);
			}
