...
class wsdl extends nusoap_base {
	...
	*/
	...
	function serializeType($name, $type, $value, $use='encoded', $encodingStyle=false, $unqualified=false)
	{
		...
		//==================由于(手机号,periodType,periodValue,periodTime)不支持，进行以下修改==============开始   by jetty
		if($name=='destinationAddresses'){
			$start = '<destinationAddresses xmlns:a="http://schemas.microsoft.com/2003/10/Serialization/Arrays" xmlns:i="http://www.w3.org/2001/XMLSchema-instance">';
			$end = '</destinationAddresses>';
			$items = '';
			foreach($value as $v) {
				$items .= "<a:string>".$v.'</a:string>';
			}
			return $start.$items.$end;
		}
		if($name=='periodType'){
			return '<periodType>'.$value.'</periodType>';
		}
		if($name=='periodValue'){
			$start = '<periodValue xmlns:a="http://schemas.datacontract.org/2004/07/OpenMas" xmlns:i="http://www.w3.org/2001/XMLSchema-instance">';
			$end = '</periodValue>';
			$items ='<a:day>'.$value[mday].'</a:day><a:month>'.$value[mon].'</a:month><a:week>'.$value[weekday].'</a:week>';
			return $start.$items.$end;
		}
		if($name=='periodTime'){
			$start = '<periodTime xmlns:a="http://schemas.datacontract.org/2004/07/OpenMas" xmlns:i="http://www.w3.org/2001/XMLSchema-instance">';
			$end = '</periodTime>';
			$items ='<a:hour>'.$value[hours].'</a:hour><a:minute>'.$value[minutes].'</a:minute><a:second>'.$value[seconds].'</a:second>';
			return $start.$items.$end;
		}


		//==================由于(手机号,periodType,periodValue,periodTime)不支持，进行以下修改==============结束
